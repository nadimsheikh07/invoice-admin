...
const MaterialDataTable = (props) => {
    ...
    const { columns, selection, refresh, dateFilter, dateFilterLabel, exportButton, exportExcel, isRowUpdate, customActions, autoRefresh } = props
    ...
    const openDateRangeFilter = () => {
        modal({
            ...
            description: <RenderDateRange open={true} handleChange={setDateRange} value={dateRange} />,
            showConfirm: true,
            showCancel: true,
        })
    }
    ...
    const exportDataExcel = async () => {
        ...
        if (tableRef.current) {
            ...
            loader({ open: true })
            await apiConfig.get(`export${props.url}`, query).then(
                result => {
    ...
    const deleteData = async (id) => {
        ...
        modal({
            ...
            loader({ open: true })
            await apiConfig.delete(`${props.url}/${id}`).then(response => {
    ...
    const importFileData = async (event) => {
        ...
        await apiConfig.post(`import${props.url}`, formData, config).then((response) => {
            if (response.status === 200) {
                if (response.data.message) {
                    snackbar({
                        message: response.data.message,
    ...
    if (props.addData) {
        actions.push({
            ...
            isFreeAction: true,
            onClick: () => {
                router.push(`${props.url}/form/new`)
                // router.push({
    ...
    if (props.editData) {
        actions.push({
            ...
            position: "row",
            onClick: (event, rowData) => {
                router.push(`${props.url}/form/${rowData.id}`)
    ...
    if (props.editPass) {
        actions.push({
            ...
            position: "row",
            onClick: (event, rowData) => {
                router.push(`${props.url}/password/${rowData.id}`)
                // router.push({
    ...
    if (props.viewData) {
        actions.push({
            ...
            onClick: (event, rowData) => {
                ...
                //     query: { id: rowData.id },
                // })
                router.push(`${props.url}/view/${rowData.id}`)
            }
        })
    }
    ...
    if (columns) {
        columns.forEach((column) => {
            let hidden = false
            if (selectedCols && selectedCols.length && !selectedCols.includes(column.field)) {
                hidden = true
            }
            ...
            switch (column.field) {
                ...
                case 'image':
                    finalColumns.push({
                        ...
                        editComponent: column.editComponent,
                        hidden: hidden,
                        render: (row) => <Avatar variant="square" src={row.image} className={classes.large} ><Icon fontSize="large" >wallpaper</Icon></Avatar>,
                    })
                    break;
                default:
                    finalColumns.push({
    ...
    actions.push({
        ...
        onClick: () => {
            modal({
                ...
                description: <SelectColumn columns={finalColumns} selectedCols={selectedCols} onSelectChange={setSelectedCols} />,
                showConfirm: true,
                showCancel: true,
            })
        }
    ...
    const customActionCall = async (id, data) => {
        loader({ open: true })
        await apiConfig.put(`/orders/${id}`, data).then((response) => {
            if (response.status == 200) {
                if (response.data.message) {
                    snackbar({
                        message: response.data.message,
    ...
    if (isRowUpdate) {
        editable.onRowUpdate = (newData, oldData) =>
            new Promise((resolve, reject) => {
                loader({ open: true })
                apiConfig.put(`${props.url}/${newData.id}`, newData).then((response) => {
                    if (response.status == 200) {
                        if (response.data.message) {
                            snackbar({
                                message: response.data.message,
            ...
            <MaterialTable
                ...
                data={query =>
                    new Promise((resolve, reject) => {
                        ...
                        apiConfig.get(`${props.url}`, { params: filter }).then(
                            result => {
                                if (result.status === 200) {
                                    resolve({
                ...
                localization={{
                    body: {
                        ...
                        filterRow: {
                            filterPlaceHolder: t('datatable:body:filterRow:filterPlaceHolder'),
