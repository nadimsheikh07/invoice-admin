...
import { Card, CardContent, CardHeader, Icon, makeStyles, Table, TableRow, TableBody, TableCell, TableHead } from '@material-ui/core';
...
const Index = (props) => {
    ...
    return (
        <form className={classes.form} noValidate>
            <Card>
                <CardHeader titleTypographyProps={{ variant: 'h6', align: 'center' }} title={label} action={<Icon onClick={() => addOption()}>add_circle_outline</Icon>} />
                <CardContent>

                    <Table>
                        <TableBody>
                            {formik.values && formik.values[name] && formik.values[name].map((option, index) => {
                                    ...
                                    <TableRow>
                                        <TableCell>
                                            {form && form.map((formData) => {
                                                ...
                                                const value = formik.values[name][index][formData.name]
                                                    ...
                                                    default:
                                                        return (
                                                            <Textbox
                                                                ...
                                                                onChange={(e) => {
                                                                    let data = formik.values
                                                                    data[name][index][formData.name] = e.target.value
                                                                    formik.setValues(data)
                                                                    formik.handleSubmit()
                                                                }}
                                                            />
                                        ...
                                        <TableCell>
                                            <Icon onClick={() => removeOption(index)}>remove_circle_outline</Icon>
                                        </TableCell>
                                    </TableRow>
                                )
                            })}
